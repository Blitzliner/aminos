...
def read_config(config_file = 'config.json', create_new_file=False):
    ...
    if not create_new_file and os.path.isfile(config_file):
        try:
        ...
        data['prefer_control'] = ''
        data['columns'] = {'sample_name': 'Sample Name'}
...
def read_raw_data(amino_names, filepath):
    ...
    if os.path.isfile(filepath):
        ...
        data = data.rename(columns=amino_names)
        _logger.info('Reorder columns according to the order of the config')
        old_col_names = list(data.columns.values)
        new_col_names = old_col_names[0:2] + list(amino_names.values())
...
def filter_raw_data(cfg, data):
    ...
    _logger.info('Sort controls ascending')
    controls[column_name] = controls[column_name].astype(str)
    controls.sort_values(column_name, axis=0, ascending=True, inplace=True)
    _logger.info('Sort data ascending')
...
def check_controls(cfg, data):
    ...
    _logger.info('Add number as postfix to control name')
...
def select_control(cfg, data):
    ...
    #_logger.info(data['checked_controls'])
...
def analyse(cfg):
    ...
    _logger.info(f'Export filename: {excel_path}')
