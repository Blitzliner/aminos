...
def write_controls_data(workbook, data, cfg):
    ...
    ws_controls.write_row(row_idx, 5, data['controls'][aoi].columns.values.tolist(), fmt_heading_orient)
    ...
    for rank, dat in enumerate(data['checked_controls']):
        ...
        if rank%2 == 0:
            ...
            ws_controls.write_row(row_idx, 5, dat['raw_data'][aoi].to_numpy(), fmt_even_row)
...
def write_patients_data(workbook, data, cfg):    
    ...
    #for (idx, patient) in data['data'].iterrows():
        ...
        for idx_as, as_name in enumerate(aoi):  # range(2, 22):                             
            ...
            fmt_num = fmt.loc[fmt.index[idx], as_name]
